...
void *thread_func1(void *arg)
{
	...
	for (i = 0; i < *(int *)arg; i++) {
		pthread_mutex_lock(&g_mutex);		// ¹ÂÅØ½º È¹µæ
		temp = g_count;
		usleep(1);
		g_count = ++temp;
		count++;
		pthread_mutex_unlock(&g_mutex);		// ¹ÂÅØ½º ¹Ý³³
		usleep(2);
	}
...
int main(int argc, char **argv)
{
	...
	ret = pthread_mutex_init(&g_mutex, NULL);		// ¹ÂÅØ½º ÃÊ±âÈ­
	if (ret != 0) {
		printf("[%d] error: %d (%d)\n", pid, ret, __LINE__);
		return EXIT_FAILURE;
	}
	...
	pthread_mutex_destroy(&g_mutex);	// ¹ÂÅØ½º Á¦°Å
	printf("[%d] terminted\n", pid);
