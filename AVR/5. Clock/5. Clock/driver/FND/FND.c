...
void FND_dispNum(uint16_t fndNum) // fndNum 값을 fnd에 출력
{
	...
	switch(findDigitState)
	{
		...
		case 1 :
		//PORTE |= (1<<7) | (1<<6) | (1<<5) | (1<<4); // 1을 내보낸다 == 불을 끌거다
		FND_NUM_PORT=fndFont[fndNum/100%10];  // fndNum의 백의 자리 추출
		FND_DIGIT_PORT &= ~(1<<FND_DIGIT_2);
		break;
		
		case 2 :
		//PORTE |= (1<<7) | (1<<6) | (1<<5) | (1<<4); // 1을 내보낸다 == 불을 끌거다
		FND_NUM_PORT=fndFont[fndNum/10%10];  // fndNum의 십의 자리 추출
		FND_DIGIT_PORT &= ~(1<<FND_DIGIT_3);
		break;
		
		case 3 :
		//PORTE |= (1<<7) | (1<<6) | (1<<5) | (1<<4); // 1을 내보낸다 == 불을 끌거다
		FND_NUM_PORT=fndFont[fndNum/1%10];  // fndNum의 일의 자리 추출
